...
subject to client approval, use complementary tools such as
...
:::{danger}
Using a personal account or personal equipment for work risks a
company or client security breach, e.g., a supply chain attack, should
personal equipment be compromised by a malicious third party.
:::
...
because GitHub does not provide mechanisms for controlling which SSH
keys or access tokens to can be used to access organizations' private
repositories.
...
devices (e.g., a PIV card, a YubiKey).  Developers **MUST NOT** store
...
authoring commits.  The Git email address is the company- or
...
hardware authentication device is **RECOMMENDED**.  The
[_YubiKey-Guide_](https://drduh.github.io/YubiKey-Guide/) provides
...
is **RECOMMENDED**.  Refer to the _YubiKey-Guide_ or the gnupg-pkcs-11
documentation referenced above for GnuPG configuration guidance.
...
:::{danger}
As of 2025-06-21, neither GitHub CLI nor GitHub Desktop encrypt stored
credentials.
:::

Developers **MUST NOT** store credentials used to access Git remote
repositories in unencrypted files, even if the underlying file system
