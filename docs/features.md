...
configuration.  For infrastructure-as-code projects, staff **SHOULD**
be able to rebuild a system from scratch given the head of the main
branch of the Git repository, the system's secrets or other runtime
...
of the main branch of the project's Git repository.  A feature branch
**SHOULD** be limited to a single business goal or product feature.
The branch name **SHOULD** succinctly describe the work, e.g.,
...
analysis.  The `--ff-only` option to the `git merge` and `git fetch`
...
the main branch with the authoritative repository.
...
repository and the authoritative repository.  This configures remote
...
branch but not yet pushed to the authoritative repository, developers
**MUST** move the work in progress to a feature branch:
...
of a repository, the original head of a feature branch may eventually
no longer be the head of the corresponding remote-tracking branch.
Pushing changes commited to the feature branch without accounting for
recent changes to the remote-tracking branch would effectively delete
other developers' work from the Git repository.  By default, Git will
handle this situation by creating a merge commit and prompting the
user to fix any conflicts (when the same file was modified in both
branches).  However, merge commits **MUST NOT** be used as they can
hide risky code changes.  Squash commits **SHOULD NOT** be used,
either, as they discard historical context useful during root cause
analysis.  Instead, the feature branch **MUST** be rebased on the head
of the remote-tracking branch prior to merging to preserve the linear
commit history of the Git repository:
...
(when the same file was modified in both branches).  However, merge
commits **MUST NOT** be used as they can hide risky code changes.
Squash commits **SHOULD NOT** be used, either, as they discard
historical context useful during root cause analysis.  Instead, the
feature branch **MUST** be rebased on the head of the main branch
...
Merged feature branches **MUST** be removed from the authoritative
repository and **SHOULD** be removed from all other clones.
...
The new head of the main branch **MUST** be tagged with the new
version number.  Refer to [Semantic Versioning](semver) and
